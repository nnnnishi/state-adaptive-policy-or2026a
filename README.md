...
サービス利用期間中のマッチング最大化問題を、離脱のみを吸収状態とする
マルコフ決定過程（MDP）として定式化します。応募から成立までにリードタイムが
あり行動時点で成立可否を観測できないため、マッチは吸収状態とせず、
1 日あたりの報酬を Σ P_user·P_comp（行動時点での最終成立の期待値）と定めます。
価値関数は**計画期間内の累積マッチング期待数**となり、期待値の線形性により
リードタイムを厳密に扱えます（マッチング確率が小さい領域ではマッチ率に近似一致）。
累積行動数 F と最終行動からの経過ステップ数 R からなる 2 次元状態に対する
価値反復法（後ろ向き帰納法）で最適方策を求めます。

## 主な特徴

- **MDP 定式化**: 離脱のみを吸収状態とし、行動時点で最終成立の期待値を報酬計上（リードタイム対応）
...
| マッチ時の扱い | ユーザを除外 | **変更**: 行動時点で期待計上しユーザは継続（リードタイム対応） |
...
- **提案手法**（状態適応的方策）: 価値反復で得た V を用い、ベルマン方程式右辺を最大化
  ...
  argmax { a_j P_comp(j) + (1−c)[a_j V(F+1, 0) + (1−a_j) V(F, R+1)] }
